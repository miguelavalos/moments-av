...
expected_build=""
expected_version=""
expected_bundle_id="${MOMENTSAV_IOS_BUNDLE_ID:-com.avalsys.momentsav}"
expected_team_id="${MOMENTSAV_APPLE_TEAM_ID:-}"

usage() {
  cat <<'USAGE'
Usage:
  scripts/check-ios-release-archive.sh --archive <MomentsAV.xcarchive>
    [--expected-build <build>] [--expected-version <version>]

Validates the final Moments AV iOS release archive before App Store Connect upload:
...
app_path="$archive_path/Products/Applications/MomentsAV.app"
...
app_dsym="$archive_path/dSYMs/MomentsAV.app.dSYM"
