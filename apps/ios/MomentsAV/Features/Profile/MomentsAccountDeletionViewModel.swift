...
@MainActor
final class MomentsAccountDeletionViewModel: ObservableObject {
    ...
    init(api: AccountDeletionAPI, signOut: @escaping () async throws -> Void) {
        self.api = api
        self.signOut = signOut
    }

    var canRequestDeletion: Bool {
        MomentsAccountDeletionPolicy.canRequestDeletion(eligibility: resolvedEligibility, confirmationText: confirmationText)
    }

    var canFinalizeDeletion: Bool {
        MomentsAccountDeletionPolicy.canFinalizeDeletion(eligibility: resolvedEligibility, summary: summary)
    }
    ...
    var canUnlinkCurrentApp: Bool {
        ...
        return MomentsAccountDeletionPolicy.canUnlinkCurrentApp(from: summary)
    }
    ...
    func load() async {
        ...
        do {
            ...
            resolvedEligibility = MomentsAccountDeletionPolicy.unavailableEligibility(copy: Self.deletionCopy)
        }
    }
    ...
    func requestDeletion() async {
        ...
        do {
            ...
            if MomentsAccountDeletionPolicy.didCompleteDeletion(eligibility: response.deleteAccountEligibility, job: response.job) {
                try await completeLocalSignOut()
                return
            }
    ...
    private func apply(summary: AccountSummary) {
        ...
        resolvedEligibility = MomentsAccountDeletionPolicy.resolvedEligibility(from: summary, copy: Self.deletionCopy)
    }
    ...
    private static var deletionCopy: MomentsAccountDeletionPolicy.Copy {
